...
<body>
    ...
    <div class="container">
        ...
        <form action="{$path}/admin/s/{$nav}/add/{if $id > 0}{$id}/{/if}" method="post" enctype="multipart/form-data">
            <input type="hidden" name="action" value="add" />

            {foreach item='item' from=$form.fields name='item'}
                {if $item.add && $item.show}
                    {if $item.type != 'hidden'}
                    ...
                    <input type="file" class="{if $item.class == 'std'}span3{elseif $item.class == 'short'}span1{elseif $item.class == 'long'}span5{/if}" id="{$item.id}" name="{$item.name}" placeholder="{$item.tip}" />
                    {elseif $item.type == 'color'}
                    <input type="color" class="{if $item.class == 'std'}span3{elseif $item.class == 'short'}span1{elseif $item.class == 'long'}span5{/if}" id="{$item.id}" name="{$item.name}" placeholder="{$item.tip}" />
